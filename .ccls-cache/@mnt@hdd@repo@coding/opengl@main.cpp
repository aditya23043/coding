...
int main() {
    ...
    // Texture
    GLfloat textureCoords[] = {1.0f, 1.0f, 1.0f, -1.0f,
                               0.0f, 1.0f, 0.0f, -1.0f};
    ...
    glBindVertexArray(
        VAO); // bind the VAO first, then bind and set VBOs and config it
    ...
    VAO can be bound just like a VBO and any subsequent vertex attrib calls from
    that point on will be stored in the VAO. When configuring vertex attrib
    pointers, we only have to make those calls once and whenever we want to draw
    the object, can just bind the corresponding VAO. This makes switching
    between different vertex data and attribute configurations as easy as
    binding a different VAO.
    And also, core openGL requires us to use VAO so it knows what to do with our
    vertex inputs
    */
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // Now instead of using glDrawArrays to draw a single triangle with the
        // given vertices We use glDrawElements to draw multiple triangles with
        // the given vertices AND indices first arg : drawing mode / primitive
        // second arg : number of elements we would like to draw i.e. 6 indices
        // here for a rectangle using two triangles third arg : type of indices
        // fourth arg : specify offset in the EBO OR pass index array when not
        // using EBO
